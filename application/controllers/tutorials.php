...
//$this->session->set_userdata('refered_from', $_SERVER[‘HTTP_REFERER’]); 
		...
		//	    $content= $content . anchor('tutorials/category/'.$val->slug,$val->menu_name).'<br/>';
...
$link='http://localhost/robotix/tutorials/category/'. $val->slug .'/' .  $val2->slug ;
...
//		$this->template->write_view('content', 'template/tutorialpage.php',$output);
	...
	/*	foreach($data['tutorials'] as $value){

	        $content = $content . anchor('tutorials/category/'.$this->uri->segment('3').'/'.$value->slug,$value->menu_name).'<br/>';

	    }

$this->template->write('title' , ' | ' . $val[0]->menu_name);
...
$this->tutorials_m->updatetut('category_id',$val[0]->id, $updatetut);
}

			 $this->tutorials_m->updatetab('category_id',$val[0]->id,array_filter(array('category'=>$data['slug'])));
