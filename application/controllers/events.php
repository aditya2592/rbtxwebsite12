...
//$output['user'] =  $this->phpbb_library->getUserInfo('username');
        ...
        //    $this->template->add_css('css/events/eventpage.less','link','screen','text/less');
           ...
           $this->template->add_css('css/events/eventpage_old.css');    
...
//$this->session->set_userdata('refered_from', uri_string()); 
        ...
        function index(){
          ...
          //  }
$this->template->write_view('content' ,  'events/events_page.php');
 $this->template->add_css('css/grid/grid.less','link','screen','text/less');
     ...
     public   function load(){ //function made for events only
        ...
        public function event(){
          ...
          $imagestrip = $this->events_m->gettabbyname($data['event'][0]->id, 'Image Strip');
$contact = $this->events_m->gettabbyname($data['event'][0]->id, 'Heads');
  $updates = $this->events_m->gettabbyname($data['event'][0]->id, 'Updates');;          

$this->template->write('image' , $imagestrip[0]->content);
         
            $this->template->write('title' , ' | '.$data['event'][0]->menu_name );

$this->template->write('contact' , $contact[0]->content);

$this->template->write('updates' , $updates[0]->content);
